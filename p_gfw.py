...
class DispatchGFW(object):
    ...
    tpl_status = pyweb.Template(template = '''<html><head><title>url list</title></head><body>length: {%=len(working)%}<br><table width="100%%"><thead><td>verb</td><td width="50%">url</td><td>action name</td><td>Elapse</td><td>from addr</td><td>send count</td><td>recv count</td></thead><tbody>{%for req, dt in working.items():%}{%dd = dtnow - dt%}{%sockaddr = '%s:%d' % (req.sock.from_addr[0], req.sock.from_addr[1])%}<tr><td>{%=req.verb%}</td><td width="50%" style="word-break:break-all;"><a href="/cutoff?from={%=sockaddr%}">{%=req.url%}</a></td><td>{%=req.app.name if hasattr(req.app, 'name') else 'unknown'%}</td><td>{%=dd.seconds%}.{%=dd.microseconds%}</td><td>{%=sockaddr%}</td><td>{%=req.trans_len[0] if hasattr(req, 'trans_len') else '0'%}</td><td>{%=req.trans_len[1] if hasattr(req, 'trans_len') else '0'%}</td></tr>{%end%}</tbody></table></body></html>''')
